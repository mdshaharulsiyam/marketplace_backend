...
const get_all = async (queryKeys: QueryKeys, searchKeys: SearchKeys) => {
  return await Aggregator(division_model, queryKeys, searchKeys, [
    {
      $project: {
        _id: 1,
        name: 1,
...
const update = async (id: string, data: { [key: string]: string }) => {
  const result = await division_model.findByIdAndUpdate(id, {
    $set: {
      ...data
    }
  }, { new: true })
...
const delete_division = async (id: string, data: { [key: string]: string }, auth: IAuth) => {
