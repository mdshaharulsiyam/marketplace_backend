...
const create = async (req: Request, res: Response) => {
  ...
  const result = await subscription_service.create(
    { subscription_id: req.body?.subscription_id, user: _id },
    packages,
    req,
  );
...
const get_my_subscription = async (req: Request, res: Response) => {
  const { _id } = req.user as IAuth;
  const result = await subscription_service.get_my_subscription(
...
const renew = async (req: Request, res: Response) => {
  const packages = req.extra?.packages;
  const { _id } = req.user as IAuth;
  const result = await subscription_service.renew(
    { subscription_id: req.body?.subscription_id, user: _id },
    packages,
    req,
  );
