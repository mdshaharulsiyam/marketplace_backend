...
favorite_router
  ...
  .get(
    "/favorite/get-all",
    verifyToken(config.USER),
    asyncWrapper(favorite_controller.get_all),
  )
  ...
  .patch(
    ...
    uploadFile(),
    asyncWrapper(favorite_controller.update),
  )

  .delete(
    "/favorite/delete/:id",
    verifyToken(config.ADMIN),
    asyncWrapper(favorite_controller.delete_favorite),
  );
